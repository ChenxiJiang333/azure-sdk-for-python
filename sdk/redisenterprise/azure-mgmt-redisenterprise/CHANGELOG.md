...
### Breaking Changes
  ...
  - Model `PrivateLinkResource` moved instance variable `required_members` and `required_zone_names` under property `properties`
  - Deleted models `AccessPolicyAssignmentList`/`ClusterList`/`DatabaseList`/`ErrorDetailAutoGenerated`/`ErrorResponseAutoGenerated`/`ProxyResourceAutoGenerated`/`ResourceAutoGenerated` which actually were not used by SDK users
