...
- Model `AppliedReservations` moved instance variable `reservation_order_ids` under property `properties`
- Model `MergeRequest` moved instance variable `sources` under property `properties`
...
- Model `SkuRestriction` renamed its instance variable `values` to `value_properties`
...
- Deleted model `AzureReservationAPI`/`CreateGenericQuotaRequestParameters`/`CurrentQuotaLimit`/`DisplayProvisioningState`/`Location`/`QuotaLimits`/`QuotaLimitsResponse`/`QuotaRequestOneResourceSubmitResponse`/`QuotaRequestSubmitResponse`/`QuotaRequestSubmitResponse201`/`RefundResponse`/`UserFriendlyAppliedScopeType`/`UserFriendlyRenewState`/`CatalogsResult`/`QuotaRequestDetailsList`/`ReservationList`/`ReservationOrderList`/`OperationList` which actually were not used by SDK users
