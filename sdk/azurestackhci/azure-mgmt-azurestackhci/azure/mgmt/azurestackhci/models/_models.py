    ...
    def __getattr__(self, name: str) -> Any:
        if name in self.__flattened_items:
            if self.component_versions is None:
                return None
            return getattr(self.component_versions, name)
        raise AttributeError(f"'{self.__class__.__name__}' object has no attribute '{name}'")

    def __setattr__(self, key: str, value: Any) -> None:
        if key in self.__flattened_items:
            if self.component_versions is None:
                self.component_versions = self._attr_to_rest_field["component_versions"]._class_type()
            setattr(self.component_versions, key, value)
        else:
            super().__setattr__(key, value)
    ...
    def __getattr__(self, name: str) -> Any:
        if name in self.__flattened_items:
            if self.package_versions is None:
                return None
            return getattr(self.package_versions, name)
        raise AttributeError(f"'{self.__class__.__name__}' object has no attribute '{name}'")

    def __setattr__(self, key: str, value: Any) -> None:
        if key in self.__flattened_items:
            if self.package_versions is None:
                self.package_versions = self._attr_to_rest_field["package_versions"]._class_type()
            setattr(self.package_versions, key, value)
        else:
            super().__setattr__(key, value)
