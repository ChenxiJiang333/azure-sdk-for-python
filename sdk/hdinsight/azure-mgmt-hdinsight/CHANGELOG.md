...
- Model `IPConfiguration` moved instance variables `provisioning_state`, `primary`, `private_ip_address`, `private_ip_allocation_method`, and `subnet` under property `properties`
- Model `PrivateEndpointConnection` moved instance variables `private_endpoint`, `private_link_service_connection_state`, `link_identifier`, and `provisioning_state` under property `properties`
- Model `PrivateLinkConfiguration` moved instance variables `group_id`, `provisioning_state`, and `ip_configurations` under property `properties`
- Model `PrivateLinkResource` moved instance variables `group_id`, `required_members`, and `required_zone_names` under property `properties`

### Other Changes

- Deleted model `ScriptActionExecutionHistoryList`/`ScriptActionsList` which actually were not used by SDK users
