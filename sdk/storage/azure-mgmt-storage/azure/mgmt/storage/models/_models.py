...
class BlobContainer(ProxyResourceAutoGenerated):
...
class BlobInventoryPolicy(ProxyResourceAutoGenerated):
...
class BlobServiceProperties(ProxyResourceAutoGenerated):
...
class DeletedAccount(ProxyResourceAutoGenerated):
...
class EncryptionScope(ProxyResourceAutoGenerated):
...
class ExecutionTriggerUpdate(_Model):
    ...
    """The trigger type of the storage task assignment execution. Known values are: \"RunOnce\" and
     \"OnSchedule\"."""
...
class FileServiceProperties(ProxyResourceAutoGenerated):
...
class FileServiceUsage(ProxyResourceAutoGenerated):
...
class FileShare(ProxyResourceAutoGenerated):
...
class ImmutabilityPolicy(ProxyResourceAutoGenerated):
...
class LocalUser(ProxyResourceAutoGenerated):
...
class ManagementPolicy(ProxyResourceAutoGenerated):
...
class ObjectReplicationPolicy(ProxyResourceAutoGenerated):
...
class QueueServiceProperties(ProxyResourceAutoGenerated):
...
class StorageAccountListKeysResult(_Model):
    ...
    keys_property: Optional[list["_models.StorageAccountKey"]] = rest_field(name="keys", visibility=["read"])
...
class StorageAccountMigration(ProxyResourceAutoGenerated):
...
class StorageAccountProperties(_Model):
    ...
    """Set the minimum TLS version to be permitted on requests to storage. The default interpretation
     is TLS 1.0 for this property. Known values are: \"TLS1_0\", \"TLS1_1\", \"TLS1_2\", and
     \"TLS1_3\"."""
    ...
    """Restrict copy to and from Storage Accounts within an AAD tenant or with Private Links to the
     same VNet. Known values are: \"PrivateLink\" and \"AAD\"."""
...
class StorageQueue(ProxyResourceAutoGenerated):
...
class StorageTaskAssignment(Resource):
    ...
    properties: "_models.StorageTaskAssignmentProperties" = rest_field(
        visibility=["read", "create", "update", "delete", "query"]
    )
    """Properties of the storage task assignment. Required."""

    @overload
    def __init__(
        self,
        *,
        properties: "_models.StorageTaskAssignmentProperties",
    ) -> None: ...
...
class StorageTaskReportInstance(ProxyResourceAutoGenerated):
...
class Table(ProxyResourceAutoGenerated):
...
class TableServiceProperties(ProxyResourceAutoGenerated):
