...
### Breaking Changes
  ...
  - Method `StorageAccountsOperations.list_keys` changed its parameter from `str` to `none`

  - Renamed enum value `ProvisioningState.ACCEPTED` to `StorageTaskAssignmentProvisioningState.ACCEPTED`
  - Renamed enum value `ProvisioningState.CANCELED` to `StorageTaskAssignmentProvisioningState.CANCELED`
  - Renamed enum value `ProvisioningState.DELETING` to `StorageTaskAssignmentProvisioningState.DELETING`
  - Renamed enum value `ProvisioningState.FAILED` to `StorageTaskAssignmentProvisioningState.FAILED`
  - Renamed enum value `ProvisioningState.VALIDATE_SUBSCRIPTION_QUOTA_BEGIN` to `StorageTaskAssignmentProvisioningState.VALIDATE_SUBSCRIPTION_QUOTA_BEGIN`
  - Renamed enum value `ProvisioningState.VALIDATE_SUBSCRIPTION_QUOTA_END` to `StorageTaskAssignmentProvisioningState.VALIDATE_SUBSCRIPTION_QUOTA_END`

  - Deleted model `BlobServiceItems`/`ErrorAdditionalInfo`/`ErrorDetail`/`ErrorResponseAutoGenerated`/`FileServiceUsages`/`FileShareItems`/`LeaseContainerRequestEnum`/`ListBlobInventoryPolicy`/`ListContainerItems`/`ListQueueResource`/`ListTableResource`/`LocalUsers`/`NetworkSecurityPerimeterConfigurationList`/`ObjectReplicationPolicies`/`ProxyResource`/`StorageTaskAssignmentsList`/`StorageTaskReportSummary` which actually were not used by SDK users

  - Method `BlobContainersOperations.create_or_update_immutability_policy`/`delete_immutability_policy`/`extend_immutability_policy`/`get_immutability_policy`/`lock_immutability_policy` replaced its parameter `if_match` to `etag`/`match_condition`

  - Model `BlobContainer` moved instance variables `version`, `deleted`, `deleted_time`, `remaining_retention_days`, `default_encryption_scope`, `deny_encryption_scope_override`, `public_access`, `last_modified_time`, `lease_status`, `lease_state`, `lease_duration`, `metadata`, `immutability_policy`, `legal_hold`, `has_legal_hold`, `has_immutability_policy`, `immutable_storage_with_versioning`, `enable_nfs_v3_root_squash` and `enable_nfs_v3_all_squash` under property `container_properties`
  - Model `BlobServiceProperties` moved instance variables `cors`, `default_service_version`, `delete_retention_policy`, `is_versioning_enabled`, `automatic_snapshot_policy_enabled`, `change_feed`, `restore_policy`, `container_delete_retention_policy` and `last_access_time_tracking_policy` under property `blob_service_properties`
  - Model `EncryptionScope` moved instance variables `source`, `state`, `creation_time`, `last_modified_time`, `key_vault_properties` and `require_infrastructure_encryption` under property `encryption_scope_properties`
  - Model `FileServiceProperties` moved instance variables `cors`, `share_delete_retention_policy` and `protocol_settings` under property `file_service_properties`
  - Model `FileShare` moved instance variables `last_modified_time`, `metadata`, `share_quota`, `provisioned_iops`, `provisioned_bandwidth_mibps`, `included_burst_iops`, `max_burst_credits_for_iops`, `next_allowed_quota_downgrade_time`, `next_allowed_provisioned_iops_downgrade_time`, `next_allowed_provisioned_bandwidth_downgrade_time`, `enabled_protocols`, `root_squash`, `version`, `deleted`, `deleted_time`, `remaining_retention_days`, `access_tier`, `access_tier_change_time`, `access_tier_status`, `share_usage_bytes`, `lease_status`, `lease_state`, `lease_duration`, `signed_identifiers`, `snapshot_time` and `file_share_paid_bursting` under property `file_share_properties`
  - Model `FileShareItem` moved instance variables `last_modified_time`, `metadata`, `share_quota`, `provisioned_iops`, `provisioned_bandwidth_mibps`, `included_burst_iops`, `max_burst_credits_for_iops`, `next_allowed_quota_downgrade_time`, `next_allowed_provisioned_iops_downgrade_time`, `next_allowed_provisioned_bandwidth_downgrade_time`, `enabled_protocols`, `root_squash`, `version`, `deleted`, `deleted_time`, `remaining_retention_days`, `access_tier`, `access_tier_change_time`, `access_tier_status`, `share_usage_bytes`, `lease_status`, `lease_state`, `lease_duration`, `signed_identifiers`, `snapshot_time` and `file_share_paid_bursting` under property `properties`
  - Model `ImmutabilityPolicy` moved instance variables `immutability_period_since_creation_in_days`, `state`, `allow_protected_append_writes` and `allow_protected_append_writes_all` under property `properties`
  - Model `ImmutabilityPolicyProperties` moved instance variables `immutability_period_since_creation_in_days`, `state`, `allow_protected_append_writes` and `allow_protected_append_writes_all` under property `properties`
  - Model `ListContainerItem` moved instance variables `version`, `deleted`, `deleted_time`, `remaining_retention_days`, `default_encryption_scope`, `deny_encryption_scope_override`, `public_access`, `last_modified_time`, `lease_status`, `lease_state`, `lease_duration`, `metadata`, `immutability_policy`, `legal_hold`, `has_legal_hold`, `has_immutability_policy`, `immutable_storage_with_versioning`, `enable_nfs_v3_root_squash` and `enable_nfs_v3_all_squash` under property `properties`
  - Model `ListQueue` moved instance variable `metadata` under property `queue_properties`
  - Model `Operation` moved instance variable `service_specification` under property `operation_properties`
  - Model `QueueServiceProperties` moved instance variable `cors` under property `queue_service_properties`
  - Model `Restriction` moved instance variable `values` under property `values_property`
  - Model `DeletedAccount` moved instance variables `storage_account_resource_id`, `location`, `restore_reference`, `creation_time` and `deletion_time` under property `properties`
  - Model `BlobInventoryPolicy` moved instance variables `last_modified_time` and `policy` under property `properties`
  - Model `LocalUser` moved instance variables `permission_scopes`, `home_directory`, `ssh_authorized_keys`, `sid`, `has_shared_key`, `has_ssh_key`, `has_ssh_password`, `user_id` and `group_id` under property `properties`
  - Model `StorageAccount` moved instance variables `provisioning_state`, `primary_endpoints`, `primary_location`, `status_of_primary`, `last_geo_failover_time`, `secondary_location`, `status_of_secondary`, `creation_time`, `custom_domain`, `sas_policy`, `key_policy`, `key_creation_time`, `secondary_endpoints`, `encryption`, `access_tier`, `azure_files_identity_based_authentication`, `enable_https_traffic_only`, `network_rule_set`, `is_sftp_enabled`, `is_local_user_enabled`, `enable_extended_groups`, `is_hns_enabled`, `geo_replication_stats`, `failover_in_progress`, `large_file_shares_state`, `private_endpoint_connections`, `routing_preference`, `dual_stack_endpoint_preference`, `blob_restore_status`, `allow_blob_public_access`, `minimum_tls_version`, `allow_shared_key_access`, `enable_nfs_v3`, `allow_cross_tenant_replication`, `default_to_o_auth_authentication`, `public_network_access`, `immutable_storage_with_versioning`, `allowed_copy_scope`, `storage_account_sku_conversion_status`, `dns_endpoint_type`, `is_sku_conversion_blocked`, `account_migration_in_progress` and `geo_priority_replication_status` under property `properties`
  - Model `StorageAccountCreateParameters` moved many instance variables (e.g., `custom_domain`, `encryption`, `network_rule_set`, `access_tier`, `azure_files_identity_based_authentication`, `enable_https_traffic_only`, `is_sftp_enabled`, `is_local_user_enabled`, `enable_extended_groups`, `is_hns_enabled`, `large_file_shares_state`, `routing_preference`, `dual_stack_endpoint_preference`, `allow_blob_public_access`, `minimum_tls_version`, `allow_shared_key_access`, `enable_nfs_v3`, `allow_cross_tenant_replication`, `default_to_o_auth_authentication`, `immutable_storage_with_versioning`, `dns_endpoint_type`, `geo_priority_replication_status`, `allowed_copy_scope`, `public_network_access`, `sas_policy`, `key_policy`) under property `properties`
  - Model `StorageAccountUpdateParameters` moved many instance variables (e.g., `custom_domain`, `encryption`, `sas_policy`, `key_policy`, `access_tier`, `azure_files_identity_based_authentication`, `enable_https_traffic_only`, `is_sftp_enabled`, `is_local_user_enabled`, `enable_extended_groups`, `network_rule_set`, `large_file_shares_state`, `routing_preference`, `dual_stack_endpoint_preference`, `allow_blob_public_access`, `minimum_tls_version`, `allow_shared_key_access`, `allow_cross_tenant_replication`, `default_to_o_auth_authentication`, `public_network_access`, `immutable_storage_with_versioning`, `allowed_copy_scope`, `dns_endpoint_type`, `geo_priority_replication_status`) under property `properties`
  - Model `StorageAccountListKeysResult` deleted or renamed instance variable `keys`
  - Model `StorageAccountMigration` moved instance variables `target_sku_name`, `migration_status`, `migration_failed_reason` and `migration_failed_detailed_reason` under property `storage_account_migration_details`
  - Model `StorageQueue` moved instance variables `metadata` and `approximate_message_count` under property `queue_properties`
  - Model `Table` moved instance variables `table_name` and `signed_identifiers` under property `table_properties`
  - Model `TableServiceProperties` moved instance variable `cors` under property `table_service_properties`
  - Model `UpdateHistoryProperty` moved instance variable `update` under property `update_property`
  ...
  - Method `FileSharesOperations.delete` changed its parameter `include`/`x_ms_snapshot` from `positional_or_keyword` to `keyword_only`
  ...
  - Method `StorageAccountsOperations.begin_failover` changed its parameter `failover_type` from `positional_or_keyword` to `keyword_only` and default value from `str` to `none`
