Follow the steps with order:
    ...
    5. search the TypeSpec code of scenario `AlternateApiVersion.Service.Header` from https://github.com/microsoft/typespec/blob/main/packages/http-specs/specs
    6. look for the test files generated from the TypeSpec code from local folder `azure/generated`
    7. find in file `azure/requirements.txt` and `unbranded/requirements.txt` to check if there is a requirement for the generated package, if no, add it in
    8. find in folder `generic_mock_api_tests` to check if there is a mock api test file sharing the same namespace, if no, create one
